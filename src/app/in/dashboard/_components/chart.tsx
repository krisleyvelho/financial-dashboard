...
import { ComposedChart, Line, Area, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, Scatter, TooltipContentProps } from 'recharts';
  ...
  if (active && payload && payload.length) {
    return (
      <div className="custom-tooltip ease-linear bg-slate-400/50 p-4 rounded-lg" style={{ color: 'black' }}>
        <p>{label}</p>
        <p>Receita Mensal: {formatCurrency(payload[0].payload.mi)}</p>
        <p>Despesas Mensais: {formatCurrency(payload[0].payload.me)}</p>
        <p>Investimentos: {formatCurrency(payload[0].payload.inv)}</p>
        <p>Taxa de Poupança: {payload[0].payload.sav.toFixed(2)}%</p>
